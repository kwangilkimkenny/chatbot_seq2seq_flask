...
def chat_start(data):
    ...
    with ChatbotModel(mode = "infer",
        ...
        while(True):
            ...
            # question = input("You: ")
            # 만약 입력한 대화가 이전에 입력한 대화와 같다면 대기한다. 이전에 입력한 대화가 저장되고 비교해야 한다. 다르면 답변을 생성한다.

            if data == question['question']: # 마지막에 추가된 값(대화)와 비교하여 같다면 이하 코드를 수행
                question  = data
                ...
                if terminate_chat:
                    ...
                    question['quesiton'] = str(data)
                    print('question add check:', question)
    return result
